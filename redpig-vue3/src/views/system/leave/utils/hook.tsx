...
export function useLeave() {
  ...
  function handleDelete(row) {
    delById("/leave/delById", { id: row.id }).then(() => {
  ...
  function delByIds() {
    ...
    ElMessageBox.confirm("确定要执行批量删除吗？", "系统提示", {
      ...
      removeBatchByIds("/leave/removeBatchByIds", ids).then(res => {
        if (res.code != 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getPageList("/leave/page", {
  ...
  function openDialog(title = "新增请假流程", row?: FormItemProps) {
    addDialog({
      title: `${title}`,
      props: {
        formInline: {
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          saveOrUpdate("/leave/saveOrUpdate", curData).then(() => {
  ...
  function openViewDialog(title = "新增请假流程", row?: FormItemProps) {
    addDialog({
      title: `${title}`,
      props: {
        formInline: {
      ...
      fullscreenIcon: true,
      closeOnClickModal: false,
      contentRenderer: () => h(viewForm, { ref: formRef }),
      beforeSure: (done, { options }) => {
        const FormRef = formRef.value.getRef();
        const curData = options.props.formInline as FormItemProps;
        function chores() {
