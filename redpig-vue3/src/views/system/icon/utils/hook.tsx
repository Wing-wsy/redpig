...
export function useIcon() {
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      width: 100,
      cellRenderer: scope => (
        <IconifyIconOnline icon={scope.row.iconTag} width="70px" height="20px" />
      )
    },
    {
      label: "创建者",
  ...
  function handleDelete(row) {
    delById("/icon/delById",{ id: row.id }).then(() => {
  ...
  function delByIds() {
    ...
    ElMessageBox.confirm("确定要执行批量删除吗？", "系统提示", {
      ...
      removeBatchByIds("/icon/removeBatchByIds",ids).then(res => {
        if (res.code != 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getPageList("/icon/page",{
  ...
  function openDialog(title = "新增文章", row?: FormItemProps) {
    addDialog({
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          saveOrUpdate("/icon/saveOrUpdate",curData).then(() => {
