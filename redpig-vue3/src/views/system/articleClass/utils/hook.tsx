...
export function useArticleClass() {
  const form = reactive({
    time: ""
  });
  const formRef = ref();
  ...
  function handleDelete(row) {
    delById("/articleClass/delById",{ id: row.id }).then(() => {
  ...
  function delByIds() {
    ...
    ElMessageBox.confirm("确定要执行批量删除吗？", "系统提示", {
      ...
      removeBatchByIds("/articleClass/removeBatchByIds",ids).then(res => {
        if (res.code != 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getPageList("/articleClass/page",{
  ...
  function openDialog(title = "新增文章类型", row?: FormItemProps) {
    addDialog({
      title: `${title}`,
      props: {
        formInline: {
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          saveOrUpdate("/articleClass/saveOrUpdate",curData).then(() => {
