...
export function usePerm() {
  ...
  function handleDelete(row) {
    delById("/perm/delById", { id: row.id }).then(() => {
  ...
  function delByIds() {
    ...
    if (ids.includes(1)) {
      ...
      ElMessageBox.confirm("确定要执行批量删除吗？", "系统提示", {
        ...
        removeBatchByIds("/perm/removeBatchByIds", ids).then(res => {
          if (res.code != 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getPageList("/perm/page", {
  ...
  function openDialog(title = "新增用户权限", row?: FormItemProps) {
    addDialog({
      title: `${title}用户权限`,
      props: {
        formInline: {
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          saveOrUpdate("/perm/saveOrUpdate", curData).then(() => {
