...
export function useExceptionLog() {
  const form = reactive({
    time: ""
  });
  const formRef = ref();
  ...
  function handleDelete(row) {
    delById("/exceptionLog/delById",{ id: row.id }).then(() => {
  ...
  function delByIds() {
    ...
    ElMessageBox.confirm("确定要执行批量删除吗？", "系统提示", {
      ...
      removeBatchByIds("/exceptionLog/removeBatchByIds",ids).then(res => {
        if (res.code != 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getPageList("/exceptionLog/page",{
  ...
  function openDialog(title = "新增日志异常记录", row?: FormItemProps) {
    addDialog({
      title: `${title}`,
      props: {
        formInline: {
            id: row?.id ?? "0" ,
            excRequParam: row?.excRequParam ?? "" ,
            operMethod: row?.operMethod ?? "" ,
            excName: row?.excName ?? "" ,
            excMessage: row?.excMessage ?? "" ,
            operUserId: row?.operUserId ?? "" ,
            operUserName: row?.operUserName ?? "" ,
            operUri: row?.operUri ?? "" ,
            operIp: row?.operIp ?? "" ,
            operCreateTime: row?.operCreateTime ?? "" ,
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          saveOrUpdate("/exceptionLog/saveOrUpdate",curData).then(() => {
