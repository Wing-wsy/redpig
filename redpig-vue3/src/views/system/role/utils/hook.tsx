...
export function useRole() {
  ...
  function handleDelete(row) {
    delById("/role/delById",{ id: row.id }).then(() => {
  ...
  function delByIds() {
    ...
    ElMessageBox.confirm("确定要执行批量删除吗？", "系统提示", {
      ...
      removeBatchByIds("/role/removeBatchByIds",ids).then(res => {
        if (res.code != 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getPageList("/role/page",{
  ...
  function openDialog(title = "新增用户角色", row?: FormItemProps) {
    addDialog({
      title: `${title}用户角色`,
      props: {
        formInline: {
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          saveOrUpdate("/role/saveOrUpdate",curData).then(() => {
