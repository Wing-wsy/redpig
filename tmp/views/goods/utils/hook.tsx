...
export function useGoods() {
  ...
  function handleDelete(row) {
    delById("/goods/delById",{ id: row.id }).then(() => {
  ...
  function delByIds() {
    ...
    ElMessageBox.confirm("确定要执行批量删除吗？", "系统提示", {
      ...
      removeBatchByIds("/goods/removeBatchByIds",ids).then(res => {
        if (res.code != 200) {
  ...
  async function onSearch() {
    loading.value = true;
    const { data } = await getPageList("/goods/page",{
  ...
  function openDialog(title = "新增商品管理", row?: FormItemProps) {
    addDialog({
      title: `${title}`,
      props: {
        formInline: {
      ...
      beforeSure: (done, { options }) => {
        ...
        function chores() {
          saveOrUpdate("/goods/saveOrUpdate",curData).then(() => {
